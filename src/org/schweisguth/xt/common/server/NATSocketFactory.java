...
public class NATSocketFactory extends RMISocketFactory {
    ...
    private static final String FIXED_SERVER_SOCKET_PORT = "fixedServerSocketPort";

    /**
     * If the requested host has an entry in the configuration, opens a socket
     * to the host given in the configuration (the NAT server) instead of to the
     * requested host (the NAT client). For this to work, the NAT client must
     * have a static address and the NAT server must map the requested port to
     * the same port on the NAT client.
     */
    public Socket createSocket(String host, int port) throws IOException {
        String actualHost = Configuration.instance().get(getClass(), NAT_SERVER + "." + host, host);
        Logger.global.fine("Creating socket to host " + actualHost + " on port " + port);
    ...
    public ServerSocket createServerSocket(int port) throws IOException {
        int fixedServerSocketPort =
            Configuration.instance().getInt(getClass(), FIXED_SERVER_SOCKET_PORT, 0);
        int actualPort = port == 0 ? fixedServerSocketPort : port;
        Logger.global.fine("Creating server socket on port " + actualPort);
